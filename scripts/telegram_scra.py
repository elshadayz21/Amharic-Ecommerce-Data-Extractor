...
import os
import csv
import re # Regular expression module for pattern matching, used to extract Amharic text
import asyncio # asyncio is a library to write concurrent code using the async/await syntax
from telethon import TelegramClient, events #telethon is a Python library to interact with Telegram's API
...
load_dotenv('.env')
api_id = os.getenv('TG_API_ID') #output the Telegram API ID: 
...
csv_file = 'telegram_scrapped_data.csv'
...
async def scrape_telegram_channels(channel):
    ...
    """
    await client.start() # Start the Telegram client for the purpose of scraping historical messages
    
    with open(csv_file, 'w', newline='', encoding='utf-8') as file: # Open the CSV file in write mode to create it
...
def start_scraping(channel):
    ...
    asyncio.run(scrape_telegram_channels(channel)) # Use asyncio.run to run the async function for historical scraping
    print("Scraping completed. Now listening for real-time messages...")
